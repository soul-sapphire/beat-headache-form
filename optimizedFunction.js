export function generatePatientReportPdf(form, fresshTotal) {
    ...
    const drawHeader = () => {
        ...
        doc.rect(0, 0, P_WIDTH, 18, "F");
        
        doc.setFont("helvetica", "bold");
        doc.setFontSize(14);
        doc.setTextColor(...C_TEXT);
        doc.text("Beat Headache", M_LEFT, 10);
        
        doc.setFont("helvetica", "normal");
        doc.setFontSize(10);
        doc.setTextColor(...C_MUTED);
        doc.text("Patient Summary", M_LEFT, 15);
    ...
    const drawFieldBox = (label, value, bx, by, bw, bh) => {
        ...
        doc.setFontSize(6.5);
        doc.setTextColor(...C_MUTED);
        doc.text(label, bx + 2, by + 3); 
        
        doc.setFont("helvetica", "normal");
        doc.setFontSize(7.5);
        doc.setTextColor(...C_TEXT);
        doc.text(truncateSmart(value, bw / 1.4), bx + 2, by + 6.5); 
    };
    ...
    drawFieldBox("Patient ID", p.registrationCode || "N/A", M_LEFT, y, bw, 9); 
    ...
    doc.roundedRect(M_LEFT, y, U_WIDTH, 12, 1, 1, "F"); 
    doc.setFont("helvetica", "bold"); doc.setFontSize(7); doc.setTextColor(...C_ACCENT);
    doc.text("Childhood / Neonatal Notes:", M_LEFT + 2, y + 4); 
    doc.setFont("helvetica", "normal"); doc.setTextColor(...C_TEXT);
    ...
    doc.text(truncateSmart(`Early Childhood Illnesses: ${med.pastMedical || "None recorded"}`, 150), M_LEFT + 2, y + 9); 
    y += 14; 

    // Past Medical + Development Cards
    const drawCard = (cx, cy, cw, title, lines) => {
        doc.setDrawColor(...C_BORDER); doc.setFillColor(...C_WHITE);
        doc.roundedRect(cx, cy, cw, 19, 1, 1, "FD"); 
        doc.setFont("helvetica", "bold"); doc.setFontSize(7); doc.setTextColor(...C_ACCENT);
        doc.text(title.toUpperCase(), cx + 2, cy + 4); 
    ...
    const hCardHeight = 33; 
    doc.setDrawColor(...C_BORDER); doc.setFillColor(...C_WHITE);
    doc.roundedRect(M_LEFT, y, U_WIDTH, hCardHeight, 1, 1, "FD"); 
    ...
    const drawFeatureBlock = (label, value, fx, fy, fw, fh) => {
        ...
        doc.text(lines.slice(0, maxLines), fx, fy + 2.5); 
    };

    // Column 1
    drawFeatureBlock("HISTORY & PATTERN", `${h.durationYears || 0}y ${h.durationMonths || 0}m | ${h.pattern || "N/A"}`, M_LEFT + 3, y + 4, colW - 6, 6); 
    drawFeatureBlock("LOCATION & SIDE", formatArraySmart(h.location, 3) + (h.frontalSide ? ` (Frontal: ${h.frontalSide})` : "") + (h.temporalSide ? ` (Temporal: ${h.temporalSide})` : ""), M_LEFT + 3, y + 11.5, colW - 6, 6); 
    drawFeatureBlock("PAIN CHARACTER", formatArraySmart(h.painNature, 3), M_LEFT + 3, y + 19, colW - 6, 6); 
    drawFeatureBlock("ASSOCIATED SYMPTOMS", formatArraySmart(h.associated, 6), M_LEFT + 3, y + 26.5, colW - 6, 8); 
    ...
    const impressionH = 46; 
    doc.setDrawColor(...C_BORDER); doc.setFillColor(...C_WHITE);
    doc.roundedRect(M_LEFT, y, cardW, impressionH, 1, 1, "FD"); 
    ...
    doc.setFont("helvetica", "bold"); doc.setFontSize(7); doc.setTextColor(...C_MUTED); doc.text("Suggested Category:", M_LEFT + 2, y + 8.5); 
    ...
    doc.setFont("helvetica", "bold"); doc.setFontSize(7); doc.setTextColor(...C_MUTED); doc.text("Classification Status:", M_LEFT + 2, y + 19); 
    ...
    if (uniqFeatures.length > 0) {
        ...
        doc.text(featLines.slice(0, 2), M_LEFT + 2, primY + 4); 
    }
    
    // Card B: Secondary Headache Screen & Red Flags
    doc.setDrawColor(...C_BORDER); doc.setFillColor(...C_WHITE);
    doc.roundedRect(M_LEFT + cardW + 4, y, cardW, impressionH, 1, 1, "FD"); 
    ...
    if (redFlags.length > 0) {
        ...
        redFlags.forEach((flag, idx) => {
            ...
            if (rfY < y + 20) { 
                ...
                doc.roundedRect(rfX, rfY, chipW, 3.5, 1, 1, "FD"); 
                doc.setTextColor(153, 27, 27);
                doc.text(flagText, rfX + 2, rfY + 2.5); 
                rfX += chipW + 2;
            }
        });
    } else {
    ...
    if (secNotes && secNotes !== "None" && secNotes.trim() !== "") {
        ...
        doc.text(secNotesLines.slice(0, 2), M_LEFT + cardW + 6, itemY + 4); 
    }
    ...
    doc.roundedRect(fx, y, 42, 6, 1, 1, "FD"); 
    doc.setFont("helvetica", "bold"); doc.setFontSize(6.5); doc.setTextColor(...C_MUTED);
    doc.text("FRESSH Total", fx + 4, y + 4); 
    doc.setFont("helvetica", "bold"); doc.setFontSize(7.5); doc.setTextColor(...C_ACCENT);
    doc.text(`${fresshTotal} / 60`, fx + 26, y + 4);
    
    // *** UPDATED: Spacing from FRESSH to Personalized Recommendations ***
    y += 12; // Increased from 8 to 12
    ...
    // *** UPDATED: Reduce card height by 15-20% and padding ***
    const recCardH = 17; // Reduced from 21
    ...
    lifestyleRecs.forEach((rec, idx) => {
        ...
        doc.setDrawColor(...C_BORDER); doc.setFillColor(...C_WHITE);
        doc.roundedRect(cx, y, recColWidth, recCardH, 1, 1, "FD");

        const { current, goal } = getRecommendation(rec.cat, rec.val);
        
        doc.setFont("helvetica", "bold"); doc.setFontSize(7); doc.setTextColor(...C_ACCENT);
        doc.text(rec.cat.toUpperCase(), cx + 2, y + 3.5); // Reduced padding from 4.5 to 3.5

        doc.setFont("helvetica", "bold"); doc.setFontSize(6.5); doc.setTextColor(...C_MUTED);
        doc.text(current, cx + 2, y + 7.5); // Reduced from 8.5 to 7.5
        ...
        doc.text(actionLines.slice(0, 3), cx + 2, y + 11.5); // Reduced from 13 to 11.5
    ...
    if (notesList.length === 0) {
        ...
        noteLines = doc.splitTextToSize(allNotes, U_WIDTH - 6).slice(0, 6);
    }
    
    // footH = padding top (6) + line heights + padding bottom (2)
    const footH = 6 + (noteLines.length * 3.2); 
    
    let footY = P_HEIGHT - M_BOTTOM - footH - 4; // Anchor securely near bottom

    // Divider before Special Notes
    doc.setDrawColor(...C_BORDER);
    doc.setLineWidth(0.5);
    doc.line(M_LEFT, footY - 3, P_WIDTH - M_RIGHT, footY - 3); 

    doc.setFillColor(...C_BG_LIGHT);
    doc.setDrawColor(...C_BORDER);
    doc.setLineWidth(0.3);
    doc.roundedRect(M_LEFT, footY, U_WIDTH, footH, 1, 1, "FD"); 
    
    doc.setFont("helvetica", "bold");
    doc.setFontSize(7.5);
    doc.setTextColor(...C_MUTED);
    doc.text("SPECIAL NOTES & OBSERVATIONS", M_LEFT + 3, footY + 4); // Adjusted title spacing
    
    doc.setFont("helvetica", "normal");
    doc.setFontSize(6.5);
    doc.setTextColor(...C_TEXT);
    
    let noteY = footY + 7.5; // Tighter padding under title
    noteLines.forEach(l => {
